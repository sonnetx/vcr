"""
Adapted ConceptAnalyzer for GLM-4.1V-9B-Thinking models.

GLM-4.1V uses the Glm4vForConditionalGeneration architecture.
This module adapts the VCR approach to work with GLM's layer structure.
"""
...
class GLM4LayerOverride(nn.Module):
    """
...
class GLM4ConceptAnalyzer:
    """
    ConceptAnalyzer adapted for GLM-4.1V-9B-Thinking models.

    Key differences from R1 ConceptAnalyzer:
    - GLM uses Glm4vForConditionalGeneration architecture
    - Different layer naming convention
    - Different processor API (apply_chat_template with tokenize=True, return_dict=True)
    """

    def __init__(self, glm4_model, clip_embedder):
        """
        Initialize analyzer with GLM-4.1V model and CLIP embedder.

        Args:
            glm4_model: GLM4VisionAPI instance
            clip_embedder: CLIPEmbedder instance for concept similarities
        """
        self.model = glm4_model
        self.model_name = glm4_model.model_id if hasattr(glm4_model, 'model_id') else 'GLM-4.1V'
        self.clip = clip_embedder
        self.image_processor = glm4_model.processor  # Use same name as original

        self.wrapped_layer = None  # The wrapped layer module (GLM4LayerOverride instance)
    ...
    def get_layer_names(self):
        """
        Get available layer names in GLM-4.1V model.

        GLM-4.1V structure (Glm4vForConditionalGeneration):
        - model.visual.blocks[i] (vision encoder, 24 blocks)
        - model.language_model.layers[i] (language model, 40 layers)
        ...
        # GLM structure: model.language_model.layers (40 layers)
        if hasattr(base_model, 'model') and hasattr(base_model.model, 'language_model'):
            if hasattr(base_model.model.language_model, 'layers'):
                num_layers = len(base_model.model.language_model.layers)
                for i in range(num_layers):
                    layers.append(f'model.language_model.layers.{i}')
        if hasattr(base_model, 'language_model'):
            if hasattr(base_model.language_model, 'layers'):
                num_layers = len(base_model.language_model.layers)
                for i in range(num_layers):
                    layers.append(f'language_model.layers.{i}')
            elif hasattr(base_model.language_model, 'model') and hasattr(base_model.language_model.model, 'layers'):
        ...
        # GLM structure: model.visual.blocks (24 blocks)
        if hasattr(base_model, 'model') and hasattr(base_model.model, 'visual'):
            if hasattr(base_model.model.visual, 'blocks'):
                num_visual_layers = len(base_model.model.visual.blocks)
        ...
        if hasattr(base_model, 'visual'):
            if hasattr(base_model.visual, 'blocks'):
                ...
                for i in range(num_visual_layers):
                    layers.append(f'visual.blocks.{i}')
            elif hasattr(base_model.visual, 'layers'):
                num_visual_layers = len(base_model.visual.layers)
                for i in range(num_visual_layers):
                    layers.append(f'visual.layers.{i}')

        return layers

    def setup_layer_hook(self, layer_name):
        """
        Set up activation hook on specified layer by wrapping it with GLM4LayerOverride.

        The layer is wrapped and replaced in the model, so all forward passes go through the wrapper.

        Args:
            layer_name: Name of layer to hook (e.g., 'model.language_model.layers.39')
        """
        self._activations = []
        ...
        self.wrapped_layer = GLM4LayerOverride(target_module)
    ...
    def collect_activations(self, dataset, system_prompt, query_template, batch_size=1):
        ...
        Args:
            ...
            system_prompt: System prompt for GLM
            query_template: Query template for GLM
    ...
    def _forward_with_completion(self, image, system_prompt, query_template, completion):
        ...
        # Process inputs with GLM's processor
        base_model = self.model.model
        if hasattr(base_model, 'module'):
            base_model = base_model.module
...
def create_glm4_analyzer(model_name='GLM-4.1V-9B-Thinking'):
    """
    Convenience function to create GLM4ConceptAnalyzer with model and CLIP.

    Args:
        model_name: GLM-4.1V model variant

    Returns:
        GLM4ConceptAnalyzer instance
    """
    from models.glm4_vision import GLM4VisionAPI
    from experiments.bootstrap_resample_for_pvalues_glm4 import CLIPEmbedder

    print(f"Loading GLM-4.1V model: {model_name}")
    glm4_model = GLM4VisionAPI(model_name=model_name)
