...
from experiments.kimi_vl_concept_analyzer import KimiVLConceptAnalyzer
...
##
## Generate top concepts for DDI dataset with Kimi-VL-A3B-Thinking
##

def run_single_seed_experiment(config_dict, df_preprocessed, random_seed, shared_data=None):
    """
    Run experiment with pre-configured dataframe for a single random seed using Kimi-VL.
    ...
    print(f"Loading Kimi-VL model: {config_dict['model_name']}")
    kimi_model = KimiVLAPI(model_name=config_dict['model_name'], use_flash_attention=False)

    print(f"Loading CLIP embedder...")
    clip = CLIPEmbedder()

    print(f"Setting up KimiVLConceptAnalyzer with layer hook...")
    analyzer = KimiVLConceptAnalyzer(kimi_model, clip)

    # Discover layer names if not specified
    layer_name = config_dict.get('layer_name')
    if layer_name is None:
        available_layers = analyzer.get_layer_names()
        print(f"Available layers: {available_layers[:5]}...")
        # Default to last language model layer
        lang_layers = [l for l in available_layers if 'layer' in l.lower() and 'vision' not in l.lower()]
        if lang_layers:
            layer_name = lang_layers[-1]
        else:
            layer_name = available_layers[-1] if available_layers else 'model.layers.27'  # Kimi-VL has ~28 layers
        print(f"Auto-selected layer: {layer_name}")
    ...
    # Use identity preprocessor since Kimi-VL will handle image processing
    ...
    for idx, image_path in enumerate(tqdm(train_paths, desc="Processing images", initial=start_idx, total=len(train_paths))):
        ...
        try:
            ...
            if config_dict['task_definition'] == 'contrastive':
                ...
                logprobs = kimi_model.get_choice_logprobs(
                    prompt=config_dict['prompt']['query_template'],
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Run VCR experiment with DDI dataset using Kimi-VL')
    parser.add_argument('--model', type=str, default='Kimi-VL-A3B-Thinking',
                       choices=['Kimi-VL-A3B-Thinking'],
                       help='Kimi-VL model to use')
    ...
    parser.add_argument('--layer', type=str, default=None,
                       help='Layer to hook for VCR analysis (default: auto-detect last language layer)')
    ...
    base_config = {
        ...
        'layer_name': args.layer,  # Layer to hook for VCR analysis (None = auto-detect)
    }
    ...
    for prompt_name, prompt_config in PROMPT_ABLATIONS.items():
        ...
        results_dir = Path(f'{args.model}_DDI_KimiVL{skin_tone_suffix}_{args.task_definition}_{prompt_name}')
        ...
        # NOTE: Different seeds have different train/test splits, so sample indices
        # refer to different images across seeds. We can only meaningfully aggregate
        # at the concept level, not at the sample level.

        # Get per-concept sensitivities for each seed (average over samples within each seed)
        # Shape: [num_seeds, num_samples, num_concepts] -> [num_seeds, num_concepts]
        per_seed_concept_sens = np.array([np.mean(sens, axis=0) for sens in all_sensitivities])

        # Average and std across seeds at the concept level
        # Shape: [num_seeds, num_concepts] -> [num_concepts]
        avg_sens_per_concept = np.mean(per_seed_concept_sens, axis=0)
        std_sens_per_concept = np.std(per_seed_concept_sens, axis=0)
        ...
        np.save(agg_dir / 'avg_sens_per_concept.npy', avg_sens_per_concept)
        np.save(agg_dir / 'std_sens_per_concept.npy', std_sens_per_concept)
        np.save(agg_dir / 'per_seed_concept_sens.npy', per_seed_concept_sens)  # [num_seeds, num_concepts]
        top_k = args.top_k_concepts
        top_indices = np.argsort(np.abs(avg_sens_per_concept))[-top_k:][::-1]
        top_concepts_overall = [concept_texts[int(i)] for i in top_indices]
    ...
    summary_path = Path(f'{args.model}_DDI_KimiVL{skin_tone_suffix}_{args.task_definition}_prompt_ablation_summary.json')
